...
class Dataset(object):
    ...
    def __init__(self,state_car_dim, state_obstacles_dim, control_dim, buffer_size=10000, safe_distance=4.0,p=0.0001):
    ...
    def add_data(self, total_state, control, total_state_next, idbf_masks=False):     
        if not idbf_masks:
            ...
            # print(dist)
            if dist<4:##was this before april 27
            ...
            # ###IMPORTANT ###IMPORTANT ###IMPORTANT ###IMPORTANT ###IMPORTANT ###IMPORTANT
            # ###DONT ADD ALL THE SAFE DATA add buffer region so tha unsafe set a bit bigger
            # elif dist>=self.safe_distance+1:
            #     self.buffer_safe.append(data)
            #     self.buffer_safe=self.buffer_safe[-self.buffer_size:]
                
            # if dist<self.safe_distance-1:
            #     self.buffer_unsafe.append(data)
            #     self.buffer_unsafe=self.buffer_unsafe[-self.buffer_size:]   
            # ###IMPORTANT ###IMPORTANT ###IMPORTANT ###IMPORTANT ###IMPORTANT ###IMPORTANT
            # ###DONT ADD ALL THE SAFE DATA add buffer region so tha unsafe set a bit bigger
            ...
            for i in range(5):
                ...
                if torch.exp(log_probs).item()<self.p:##won't matter much but sample from the policy itself not from uniform
