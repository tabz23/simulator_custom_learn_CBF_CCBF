...
class Trainer():
    ...
    def get_mask(self, state):
        dist = torch.norm(state[:,:2] - state[:,2:4], dim=1)
        safe_mask = dist > self.safe_distance#becomes 5
        unsafe_mask = dist <= self.safe_distance - 1 #becomes 4
        return safe_mask, unsafe_mask
    ...
    def train_cbf(self):
        ...
        for i in range(self.opt_iter):
            ...
            if not self.idbf_data:
                safe_mask, unsafe_mask = self.get_mask(state)##can be removed, doesn't affect anything, can remove later since already our dataset returns 32 safe then 32 unsafe samples
                
            else:##if idbf training then just use the fact that dataset already creates first 32 samples as safe and second 32 as unsafe (where unsafe are determined using Safe BC controller, please check dataset file for more details), assuming there are more than batchsize/2 unsafe samples in dataset buffer, which is always true given sufficiently large p threshold for idbf
            ...
            dist_next = torch.norm(next_state[:,:2] - next_state[:,2:4], dim=1)##next state is safe then we have safe action and can use loss of gradient
            safe_mask_deriv = dist_next > self.safe_distance
            num_safe_next = torch.sum(safe_mask_deriv).float()
            
            deriv_cond = h_dot + h
            loss_deriv_safe = torch.sum(relu(-deriv_cond).reshape(-1, 1) * safe_mask_deriv.reshape(-1, 1)) / (num_safe_next + 1e-5)
