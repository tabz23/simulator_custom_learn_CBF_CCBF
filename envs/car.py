...
class DubinsCarEnv:
    def __init__(self, max_velocity, dt, obstacle_radius=None, goal_position=None):
        ...
        # self.obstacle_radius = obstacle_radius if obstacle_radius else 4.0 ##was like this before, i think the below is better since 0 might be treated as bool
        self.obstacle_radius = obstacle_radius if (obstacle_radius==None) else 4.0
    ...
    def step(self, action):
        ...
        #ADD SOME NOISE
        # if np.random.rand() < 0.1: ##COMMENTED BECAUSE NOT USING IT TO GENERATE DATASET
        ...
        goal_reached = np.linalg.norm(self.goal_position - self.state) < 0.5
    ...
    def render(self):
        ...
        self.ax.add_patch(plt.Circle(self.obstacle_position, self.obstacle_radius, color='g', alpha=0.5, label="Obstacle"))
        ...
        if self.collision:
            self.ax.set_title("CCBF-based Navigation Simulation - COLLISION!")
            self.ax.set_facecolor((1.0, 0.9, 0.9))  # Light red background
        else:
            self.ax.set_title("CCBF-based Navigation Simulation",fontsize=14, fontweight='bold')
            self.ax.set_facecolor((1.0, 1.0, 1.0))  # White background

        self.ax.set_xlabel("X Position",fontsize=20)
        self.ax.set_ylabel("Y Position",fontsize=20)
        self.ax.legend(fontsize=14)  # You can change 14 to any preferred size
